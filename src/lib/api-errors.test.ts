...
const CASES: Array<[code: string, status: number, message: string]> = [
  ...
  [
    ...
    409,
    "Изменение вернёт долг вышедшему участнику — сначала верните его в группу",
  ],
  ["MEMBER_ALREADY_ACTIVE", 409, "Пользователь уже состоит в группе"],
  ["GROUP_MEMBER_LIMIT", 409, "В группе уже максимальное число участников"],
  [
    "ADMIN_CANNOT_LEAVE",
    409,
  ...
  ["CONVERTED_AMOUNT_TOO_SMALL", 422, "Сумма после пересчёта меньше минимальной единицы валюты группы"],
  ...
  [
    ...
    409,
    "Результат исходного запроса больше недоступен",
  ],
  ["TRANSACTION_CONFLICT", 409, "Данные изменились одновременно — повторите действие"],
...
describe("handleServiceError", () => {
  ...
  it("контракт фиксирует 26 доменных кодов (изменение набора — осознанное)", () => {
