...
CREATE INDEX IF NOT EXISTS "group_members_userId_isActive_idx"
  ON "group_members" ("userId", "isActive");

-- Expense: fast paginated list sorted by date per group
CREATE INDEX IF NOT EXISTS "expenses_groupId_date_idx"
  ON "expenses" ("groupId", "date" DESC);

-- Settlement: fast sorted list per group
CREATE INDEX IF NOT EXISTS "settlements_groupId_date_idx"
